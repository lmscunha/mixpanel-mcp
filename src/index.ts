...
server.tool(
  ...
  "Get today's top events from Mixpanel. Useful for quickly identifying the most active events happening today, spotting trends, and monitoring real-time user activity.",
  {
    ...
    type: z
      ...
      .describe(
        "The type of events to fetch, either general, average, or unique, defaults to general",
      )
      .optional(),
...
server.tool(
  ...
  "Get data for a profile's event activity. Useful for understanding individual user journeys, troubleshooting user-specific issues, and analyzing behavior patterns of specific users.",
  {
    ...
    workspace_id: z
      .string()
      .describe("The ID of the workspace if applicable")
      .optional(),
    distinct_ids: z
      .string()
      .describe(
        'A JSON array as a string representing the `distinct_ids` to return activity feeds for. Example: `["12a34aa567eb8d-9ab1c26f345b67-89123c45-6aeaa7-89f12af345f678"]`',
      ),
    from_date: z
      .string()
      .describe(
  ...
  async ({
    ...
    try {
      ...
      // Construct URL with query parameters
      let url = `https://eu.mixpanel.com/api/query/stream/query?project_id=${project_id}&distinct_ids=${encodeURIComponent(distinct_ids)}&from_date=${from_date}&to_date=${to_date}`;
...
server.tool(
  ...
  async ({ project_id = DEFAULT_PROJECT_ID, type = "general", limit = 10 }) => {
    try {
      ...
      // Construct URL with query parameters
      const url = `https://eu.mixpanel.com/api/query/events/names?project_id=${project_id}&type=${type}${limit ? `&limit=${limit}` : ""}`;
...
server.tool(
  ...
  "Get unique, general, or average data for a set of events over N days, weeks, or months. Useful for trend analysis, comparing event performance over time, and creating time-series visualizations.",
  {
    ...
    event: z
      .string()
      .describe(
        'The event or events that you wish to get data for, a string encoded as a JSON array. Example format: "["play song", "log in", "add playlist"]"',
      ),
    type: z
      .enum(["general", "unique", "average"])
      .describe(
        "The type of data to fetch, either general, unique, or average, defaults to general",
      )
      .optional(),
    unit: z
      .enum(["minute", "hour", "day", "week", "month"])
      .describe("The level of granularity of the data you get back"),
    interval: z
      .number()
      .optional()
      .describe(
  ...
  async ({
    ...
    try {
      ...
      if (!interval && (!from_date || !to_date)) {
        throw new Error(
          "You must specify either interval or both from_date and to_date",
        );
      }
      ...
      // Construct URL with query parameters
      const url = `https://eu.mixpanel.com/api/query/events?${queryParams.toString()}`;
...
server.tool(
  ...
  "Get unique, general, or average data for a single event and property over days, weeks, or months. Useful for analyzing how specific properties affect event performance, segmenting users, and identifying valuable user attributes.",
  {
    ...
    name: z
      .string()
      .describe("The name of the property you would like to get data for"),
    values: z
      .string()
      .optional()
      .describe(
    ...
    type: z
      .enum(["general", "unique", "average"])
      .describe(
        "The analysis type - general, unique, or average events, defaults to general",
      )
      .optional(),
    unit: z
      .enum(["minute", "hour", "day", "week", "month"])
    ...
    limit: z
      .number()
      .optional()
      .describe("The maximum number of values to return (default: 255)"),
  },
  async ({
    project_id = DEFAULT_PROJECT_ID,
    event,
    ...
    try {
      ...
      if (values) {
        try {
          ...
          throw new Error(
            `Invalid values format: ${e instanceof Error ? e.message : String(e)}`,
          );
        }
      }
      ...
      // Construct URL with query parameters
      const url = `https://eu.mixpanel.com/api/query/events/properties?${queryParams.toString()}`;
...
server.tool(
  ...
  async ({ project_id = DEFAULT_PROJECT_ID, workspace_id, bookmark_id }) => {
    try {
      ...
      const url = `https://eu.mixpanel.com/api/query/insights?${queryParams.toString()}`;
...
server.tool(
  ...
  "Get data for a funnel based on a funnel_id. Useful for analyzing user conversion paths, identifying drop-off points in user journeys, and optimizing multi-step processes. Funnel IDs should be retrieved using the list_saved_funnels tool. ",
  {
    ...
    funnel_id: z
      .string()
      .describe("The Mixpanel funnel ID that you wish to get data for"),
    from_date: z
      .string()
      .describe(
        "The date in yyyy-mm-dd format to begin querying from (inclusive)",
    ...
    length: z
      .number()
      .optional()
      .describe("The number of units each user has to complete the funnel"),
    length_unit: z
      .enum(["day", "hour", "minute", "second"])
      .optional()
      .describe("The unit applied to the length parameter"),
    interval: z
      .number()
      .optional()
      .describe("The number of days you want each bucket to contain"),
    unit: z
      .enum(["day", "week", "month"])
      .optional()
      .describe("Alternate way of specifying interval"),
  },
  async ({
    project_id = DEFAULT_PROJECT_ID,
    workspace_id,
    ...
    try {
      ...
      const url = `https://eu.mixpanel.com/api/query/funnels?${queryParams.toString()}`;
...
server.tool(
  ...
  async ({ project_id = DEFAULT_PROJECT_ID, workspace_id }) => {
    try {
      ...
      const url = `https://eu.mixpanel.com/api/query/funnels/list?${queryParams.toString()}`;
...
server.tool(
  ...
  async ({ project_id = DEFAULT_PROJECT_ID, workspace_id }) => {
    try {
      ...
      const url = `https://eu.mixpanel.com/api/query/cohorts/list?${queryParams.toString()}`;
...
server.tool(
  ...
  "Get data from your Retention reports. Useful for analyzing user engagement over time, measuring product stickiness, and understanding how well your product retains users after specific actions. Only use params interval or unit, not both.",
  {
    ...
    born_event: z
      ...
      .optional()
      .describe(
        "The first event a user must do to be counted in a birth retention cohort, required if retention_type is 'birth'. Can use $mp_web_page_view as the born_event for general cases.",
      ),
    event: z
      .string()
      .optional()
    ...
    born_where: z.string().optional()
      .describe(`An expression to filter born_events by based on the grammar: <expression> ::= 'properties["' <property> '"]'
   ...
   <binary op> ::= '+' | '-' | '*' | '/' | '%' | '==' | '!=' |
                  '>' | '>=' | '<' | '<=' | 'in' | 'and' | 'or'
                | <unary op> ::= '-' | 'not'`),
    return_where: z.string().optional()
      .describe(`An expression to filter return events by based on the grammar: <expression> ::= 'properties["' <property> '"]'
                | <expression> <binary op> <expression>
                | <unary op> <expression>
                | <math op> '(' <expression> ')'
                | <string literal>
   <binary op> ::= '+' | '-' | '*' | '/' | '%' | '==' | '!=' |
    ...
    interval: z
      ...
      .optional()
      .describe(
        "The number of units per individual bucketed interval. Default is 1. DO NOT USE IF ALREADY PROVIDING UNIT.",
      ),
    interval_count: z
      .number()
      .optional()
      .describe(
        "The number of individual buckets/intervals to return. Default is 1. DO NOT USE IF ALREADY PROVIDING UNIT.",
      ),
    unit: z
      .enum(["day", "week", "month"])
      .optional()
      .describe(
        "The interval unit: 'day' (eg use if asked for D7 or D30), 'week' (eg use if asked for W12), or 'month' (eg use if asked for M6). Default is 'day'. DO NOT USE IF ALREADY PROVIDING INTERVAL.",
      ),
    on: z
      .string()
      .optional()
      .describe("The property expression to segment the second event on"),
    limit: z
      .number()
      .optional()
      .describe(
  ...
  async ({
    ...
    try {
      ...
      if (interval) queryParams.append("interval", interval.toString());
      if (interval_count)
        queryParams.append("interval_count", interval_count.toString());
      if (unit) queryParams.append("unit", unit);
      if (on) queryParams.append("on", on);
      if (limit) queryParams.append("limit", limit.toString());

      const url = `https://eu.mixpanel.com/api/query/retention?${queryParams.toString()}`;
...
server.tool(
  ...
  async ({ project_id = DEFAULT_PROJECT_ID, workspace_id, script, params }) => {
    try {
      ...
      const url = `https://eu.mixpanel.com/api/query/jql?${queryParams.toString()}`;
...
server.tool(
  ...
  "Sum a numeric expression for events over time. Useful for calculating revenue metrics, aggregating quantitative values, and tracking cumulative totals across different time periods.",
  {
    ...
    unit: z
      .enum(["hour", "day"])
      .optional()
      .describe("Time bucket size: 'hour' or 'day'. Default is 'day'"),
    where: z.string().optional()
      .describe(`An expression to filter events by based on the grammar: <expression> ::= 'properties["' <property> '"]'
                | <expression> <binary op> <expression>
                | <unary op> <expression>
                | <math op> '(' <expression> ')'
                | <string literal>
  ...
  async ({
    ...
    try {
      ...
      const url = `https://eu.mixpanel.com/api/query/segmentation/sum?${queryParams.toString()}`;
...
server.tool(
  ...
  "Query Mixpanel user profiles with filtering options. Useful for retrieving detailed user profiles, filtering by specific properties, and analyzing user behavior across different dimensions.",
  {
    project_id: z
      .string()
      .describe(
        "The Mixpanel project ID. Optional since it has a default.. Optional since it has a default.",
      )
      .optional(),
    workspace_id: z
      .string()
    ...
    distinct_id: z
      .string()
      .describe("A unique identifier used to distinguish an individual profile")
      .optional(),
    distinct_ids: z
      .string()
      .describe(
        'A JSON array of distinct_ids to retrieve profiles for. Example: \'["id1", "id2"]\'',
      )
      .optional(),
    data_group_id: z
      .string()
      .describe("The ID of the group key, used when querying group profiles")
      .optional(),
    where: z
      .string()
      .describe(
        `An expression to filter users (or groups) by. Using the following grammar: <expression> ::= 'properties["' <property> '"]'
                | <expression> <binary op> <expression>
                | <unary op> <expression>
                | <math op> '(' <expression> ')'
...
<typecast op> ::= 'boolean' | 'number' | 'string'
   <property> ::= 'properties["' <property name> '"]'`,
    ...
    output_properties: z
      .string()
      .describe(
        'A JSON array of names of properties you want returned. Example: \'["$last_name", "$email", "Total Spent"]\'',
      )
      .optional(),
    session_id: z
      .string()
    ...
    behaviors: z
      .number()
      .describe(
        "If you are exporting user profiles using an event selector, you use a 'behaviors' parameter in your request",
      )
      .optional(),
    as_of_timestamp: z
      .number()
      .describe("This parameter is only useful when also using 'behaviors'")
      .optional(),
    filter_by_cohort: z
      .string()
      .describe(
        "Takes a JSON object with a single key called 'id' whose value is the cohort ID. Example: '{\"id\":12345}'",
      )
      .optional(),
    include_all_users: z
      .boolean()
      .describe(
        "Only applicable with 'filter_by_cohort' parameter. Default is true",
      )
      .optional(),
  },
  async ({
    ...
    try {
      ...
      let url = `https://eu.mixpanel.com/api/query/engage?project_id=${project_id}`;
      ...
      if (output_properties)
        formData.append("output_properties", output_properties);
      ...
      if (as_of_timestamp !== undefined)
        formData.append("as_of_timestamp", as_of_timestamp.toString());
      if (filter_by_cohort)
        formData.append("filter_by_cohort", filter_by_cohort);
      if (include_all_users !== undefined)
        formData.append("include_all_users", include_all_users.toString());
...
server.tool(
  ...
  "Get data for frequency of actions over time. Useful for analyzing how often users perform specific actions, identifying patterns of behavior, and tracking user engagement over time.",
  {
    ...
    addiction_unit: z
      .enum(["hour", "day"])
      .describe("The granularity to return frequency of actions at"),
    event: z
      .string()
      .describe("The event to generate returning counts for")
      .optional(),
    where: z.string()
      .describe(`An expression to filter the returning events by based on the grammar: <expression> ::= 'properties["' <property> '"]'
                | <expression> <binary op> <expression>
                | <unary op> <expression>
                | <math op> '(' <expression> ')'
                | <string literal>
  ...
  async ({
    ...
    try {
      ...
      let url = `https://eu.mixpanel.com/api/query/retention/addiction?project_id=${project_id}&from_date=${from_date}&to_date=${to_date}&unit=${unit}&addiction_unit=${addiction_unit}`;
...
server.tool(
  ...
  "Get data for an event, segmented and filtered by properties. Useful for breaking down event data by user attributes, comparing performance across segments, and identifying which user groups perform specific actions.",
  {
    ...
    on: z
      .string()
      .describe("The property expression to segment the event on")
      .optional(),
    unit: z
      .enum(["minute", "hour", "day", "month"])
      .describe(
        "The buckets into which the property values that you segment on are placed. Default is 'day'",
      )
      .optional(),
    interval: z
      .number()
      .describe(
        "Optional parameter in lieu of 'unit' when 'type' is not 'general'. Determines the number of days your results are bucketed into",
      )
      .optional(),
    where: z.string()
      .describe(`An expression to filter events by based on the grammar: <expression> ::= 'properties["' <property> '"]'
   ...
   <binary op> ::= '+' | '-' | '*' | '/' | '%' | '==' | '!=' |
    ...
    type: z
      .enum(["general", "unique", "average"])
      .describe(
        "The type of analysis to perform, either general, unique, or average, defaults to general",
      )
      .optional(),
  ...
  async ({
    ...
    try {
      ...
      // Construct base URL with required parameters
      let url = `https://eu.mixpanel.com/api/query/segmentation?project_id=${project_id}&event=${encodeURIComponent(event)}&from_date=${from_date}&to_date=${to_date}`;
...
server.tool(
  ...
  "Get data for an event, segmented and filtered by properties, with values placed into numeric buckets. Useful for analyzing distributions of numeric values, creating histograms, and understanding the range of quantitative metrics.",
  {
    ...
    on: z
      .string()
      .describe(
        "The property expression to segment the event on. This expression must be a numeric property",
      ),
    unit: z
      .enum(["hour", "day"])
      .describe(
  ...
  async ({
    ...
    try {
      ...
      // Construct base URL with required parameters
      let url = `https://eu.mixpanel.com/api/query/segmentation/numeric?project_id=${project_id}&event=${encodeURIComponent(event)}&from_date=${from_date}&to_date=${to_date}&on=${encodeURIComponent(on)}`;
...
server.tool(
  ...
  "Averages an expression for events per unit time. Useful for calculating average values like purchase amounts, session durations, or any numeric metric, and tracking how these averages change over time.",
  {
    ...
    unit: z
      .enum(["hour", "day"])
      .describe(
        "The buckets [hour, day] into which the property values are placed. Default is 'day'",
      )
      .optional(),
    where: z
      .string()
  ...
  async ({
    ...
    try {
      ...
      // Construct base URL with required parameters
      let url = `https://eu.mixpanel.com/api/query/segmentation/average?project_id=${project_id}&event=${encodeURIComponent(event)}&from_date=${from_date}&to_date=${to_date}&on=${encodeURIComponent(on)}`;
...
server.tool(
  ...
  "Get the top property names for an event. Useful for discovering which properties are most commonly associated with an event, prioritizing which dimensions to analyze, and understanding event structure.",
  {
    ...
    limit: z
      .number()
      .describe("The maximum number of properties to return. Defaults to 10")
      .optional(),
  },
  async ({ project_id = DEFAULT_PROJECT_ID, workspace_id, event, limit }) => {
    try {
      // Create authorization header using base64 encoding of credentials
      const credentials = `${SERVICE_ACCOUNT_USER_NAME}:${SERVICE_ACCOUNT_PASSWORD}`;
      const encodedCredentials = Buffer.from(credentials).toString("base64");
      ...
      // Construct base URL with required parameters
      let url = `https://eu.mixpanel.com/api/query/events/properties/top?project_id=${project_id}&event=${encodeURIComponent(event)}`;
...
server.tool(
  ...
  "Get the top values for a property. Useful for understanding the distribution of values for a specific property, identifying the most common categories or segments, and planning further targeted analyses.",
  {
    ...
    limit: z
      .number()
      .describe("The maximum number of values to return. Defaults to 255")
      .optional(),
  },
  async ({
    project_id = DEFAULT_PROJECT_ID,
    ...
    try {
      ...
      // Construct base URL with required parameters
      let url = `https://eu.mixpanel.com/api/query/events/properties/values?project_id=${project_id}&event=${encodeURIComponent(event)}&name=${encodeURIComponent(name)}`;
